# DOCXライブラリ チュートリアル（原文併記）

> **注（重要）**: この文書は `.docx` を壊さず生成するための**クリティカルな落とし穴**が多数あります。コマンド/コード/定数値は正確性のため原文を保持しつつ、要点を日本語で補足しています。  
> **必ず**このドキュメントを最初から最後まで読んでから着手してください（途中読み飛ばしは、破損ファイルや表示崩れの原因になります）。
