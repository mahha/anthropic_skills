# PowerPoint向け Office Open XML 技術リファレンス（原文併記）

> **注（重要）**: PPTXのOOXMLは要素順序・必須属性などの要件が厳しく、少しのミスでPowerPointが開けないファイルになります。  
> このJP版では、誤りやすいポイントを日本語で補足しつつ、XML断片・属性名・コードは正確性のため原文を保持します。  
> **必ず全文を読んでから開始**してください。
